...
# Easy to Say, Hard to Recover: Code and Data
...
* `webnlg_fewshot_verbalisation_benchmark_cycle.py`:
* Implements the cyclic evaluation (RDF → text → RDF) where model-generated inputs are used instead of ground truth.
* Requires passing the target CSV file via `--input_csv path/to/generations... .csv`.
...
* `webnlg_fewshot_ie_benchmark_cycle.py`:
* Implements the cyclic evaluation for IE (text → RDF → text), substituting ground truth references with model-generated verbalisations.
* Requires passing the target CSV file via `--input_csv path/to/generations... .csv`.
...
3. (Optional) Run the cyclic evaluation:
```bash
python verbalisation/webnlg_fewshot_verbalisation_benchmark_cycle.py --input_csv path/to/generations.csv

```


4. Open `verbalisation/evaluation/webnlg_generation_evaluation_bertscore_prf_bleu.ipynb` and run the notebook to compute metrics and write summaries to the `evaluation/` folder.
...
3. (Optional) Run the cyclic evaluation:
```bash
python IE/Few-shot/webnlg_fewshot_ie_benchmark_cycle.py --input_csv path/to/generations.csv

```


4. Open `IE/Few-shot/evaluation/webnlg_ie_evaluation_metrics.ipynb` to compute IE metrics, hallucination/omission rates, and graph similarity, and export the aggregated CSV/XLSX files.
