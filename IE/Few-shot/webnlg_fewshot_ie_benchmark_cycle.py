...
# We only need the train split from the XML to get the few-shot anchors
PARSE_SPLITS = ["train"] 
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Few-shot WebNLG IE benchmark from verbalization CSV")
    parser.add_argument("--input_csv", type=Path, required=True, help="Input CSV containing verbalizations")
    parser.add_argument("--data_root", type=Path, default=DEFAULT_DATA_ROOT)
    parser.add_argument("--output_dir", type=Path, default=Path("./outputs_ie_pipeline"))
...
def output_csv_path(output_dir: Path, model_name: str) -> Path:
    return output_dir / f"ie_generations__{model_slug(model_name)}__Qwen__Qwen3-4B-Instruct-2507.csv"
...
def prepare_dataset(data_root: Path, input_csv: Path, seed: int) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, List[pd.Series]], pd.DataFrame]:
    ...
    for lang in TARGET_LANGS:
        for split in PARSE_SPLITS: # Now just ["train"]
    ...
    # 2. Parse INPUT CSV to get target texts for Information Extraction
    print(f"\nLoading verbalizations from {input_csv}...")
    run_df_raw = pd.read_csv(input_csv)
    
    # Rename verbalization columns to avoid collision with new IE metrics
    rename_map = {
        "model_name": "verb_model_name",
        "raw_generation": "verb_raw_generation",
        "generation_status": "verb_generation_status",
        "generation_error": "verb_generation_error",
        "latency_sec": "verb_latency_sec",
        "timestamp_utc": "verb_timestamp_utc",
        "prompt": "verb_prompt",
        "messages": "verb_messages"
    }
    run_df_raw.rename(columns={k: v for k, v in rename_map.items() if k in run_df_raw.columns}, inplace=True)
    ...
    for _, row in run_df_raw.iterrows():
        ...
        # Get target text directly from previous generation
        target_text = row_dict.get("extracted_verbalization", "")
        if pd.isna(target_text) or not isinstance(target_text, str):
            target_text = ""
            
        prompt = build_user_prompt(lang, fewshot_examples[lang], target_text)
        messages = build_messages(lang, prompt)
        
        row_dict["prompt"] = prompt
        row_dict["messages"] = json.dumps(messages, ensure_ascii=False)
...
def run_generation_for_model(
    ...
    if "verb_model_name" in run_df.columns:
        key_cols.append("verb_model_name")
    ...
    if out_path.exists() and not args.overwrite_existing:
        ...
        if not existing_df.empty:
            required_cols = set(key_cols + ["ie_generation_status"])
            if required_cols.issubset(existing_df.columns):
                completed_df = existing_df[existing_df["ie_generation_status"] == "ok"].copy()
                completed_keys = set(
    ...
    try:
        for idx, (_, row) in enumerate(
            ...
            new_results.append(
                {
                    **row.drop(labels=["prompt_len_chars"]).to_dict(),
                    "ie_model_name": model_name,
                    "ie_raw_generation": raw_generation,
                    "predicted_triples": json.dumps(predicted_triples, ensure_ascii=False),
                    "extracted_triples_text": extracted,
                    "ie_generation_status": status,
                    "ie_generation_error": error,
                    "ie_latency_sec": round(time.time() - t0, 4),
                    "ie_timestamp_utc": pd.Timestamp.utcnow().isoformat(),
                }
            )
            ...
            if idx % args.save_every == 0:
                ...
                merged_df = (
                    merged_df.sort_values("ie_timestamp_utc")
        ...
        # Keep latest result per key in case of duplicates
        result_df = (
            result_df.sort_values("ie_timestamp_utc")
            .drop_duplicates(subset=key_cols, keep="last")
        )
...
def main() -> None:
    ...
    # Check if we have multiple verbalizers in the CSV
    print_cols = ["lang", "split", "category", "eid", "size", "num_triples"]
    if "verb_model_name" in run_df.columns:
        print_cols.insert(0, "verb_model_name")
    ...
    combined_path = args.output_dir / "ie_generations_all_models.csv"
    results_df.to_csv(combined_path, index=False)
    print(f"\nCombined results: {combined_path}")

    # Set up group columns for final validation
    group_cols = ["ie_model_name", "lang", "split"]
    if "verb_model_name" in results_df.columns:
        group_cols.insert(1, "verb_model_name")

    qc = (
        results_df.groupby(group_cols)
        .agg(
            rows=("align_key", "size"),
            ok=("ie_generation_status", lambda s: (s == "ok").sum()),
            nonempty=("extracted_triples_text", lambda s: s.fillna("").str.len().gt(0).sum()),
            avg_latency_sec=("ie_latency_sec", "mean"),
        )
        .reset_index()
    )
    print("\nQuick sanity check:")
